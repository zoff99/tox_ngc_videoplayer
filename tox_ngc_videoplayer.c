 ...
 *
 * https://github.com/zoff99/tox_ngc_videoplayer
 *
 *
 */
/*
 ...
 *
 ...
 ASAN compile:
 gcc -O3 -g -fno-omit-frame-pointer -fsanitize=address -static-libasan -fPIC -Wno-discarded-qualifiers tox_ngc_videoplayer.c $(pkg-config --cflags --libs x11 libsodium libswresample opus vpx libavcodec libswscale libavformat libavdevice libavutil x264) -pthread -o tox_ngc_videoplayer

 NORMAL compile:
 gcc -O3 -flto -fomit-frame-pointer -g -march=native -fPIC -Wno-discarded-qualifiers tox_ngc_videoplayer.c $(pkg-config --cflags --libs x11 libsodium libswresample opus vpx libavcodec libswscale libavformat libavdevice libavutil x264) -pthread -o tox_ngc_videoplayer

 MOSTLY STATIC compile:
 gcc -O3 -fomit-frame-pointer -Wno-discarded-qualifiers -fPIC tox_ngc_videoplayer.c \
  -Wl,-Bstatic \
                                 ...
                                 const uint8_t *group_name, size_t group_name_length, void *userdata)
{
    ...
    tox_group_invite_accept(tox, friend_number, invite_data, length,
                                 ...
                                 NULL, 0,
                                 &error);
    printf("tox_group_invite_accept:%d\n", error);
}
...
*/
    ...
    *buf = 224; // 480; // LOL
    buf++;
    // warning: unsigned conversion from ‘int’ to ‘uint8_t’ {aka ‘unsigned char’} changes value from ‘640’ to ‘128’
    *buf = 128; // 640; // LOL
    buf++;
    *buf = 0x01;
    buf++;
                            ...
                            TOXAV_ERR_SEND_FRAME *error, int32_t age_ms)
{
    ...
    global_last_sent_video_frame = current_time_monotonic_default2();
 ...
 */
static void calculateBoundingBox(int in_width, int in_height, int *out_width, int *out_height)
{
    const double aspectRatio = (double)in_width / (double)in_height;
    if (aspectRatio > (16.0 / 9.0)) {
        *out_width = NGC_VIDEO_OUT_WIDTH;
        *out_height = (int)(NGC_VIDEO_OUT_WIDTH / aspectRatio);
    } else {
        *out_height = NGC_VIDEO_OUT_HEIGHT;
        *out_width = (int)(NGC_VIDEO_OUT_HEIGHT * aspectRatio);
    }
}
...
static void *ffmpeg_thread_video_func(void *data)
{
    ...
    output_width = NGC_VIDEO_OUT_WIDTH;
    output_height = NGC_VIDEO_OUT_HEIGHT;
...
int main(int argc, char *argv[])
{
    ...
    tox_self_set_name(tox, "ToxNGCVideoplayer", strlen("ToxNGCVideoplayer"), NULL);
    ...
    // HINT: set these to start sending video -----------
    friend_in_call = 1;
    friend_online = 1;
    // HINT: set these to start sending video -----------
